...
## 2024-05-29 - Actionable Error Recovery
**Learning:** Transient errors (like network failures during profile creation or chat initialization) shown via simple `SnackBar` messages force users to manually re-initiate the action (e.g., clicking "Continue" again). This creates friction in recovery. Adding a native `SnackBarAction` like "RETRY" directly to the error message significantly reduces this friction and empowers keyboard/screen-reader users with an immediate, focused recovery path.
**Action:** When displaying transient or network-related error SnackBars, always include an actionable `SnackBarAction` (like 'RETRY') that immediately re-triggers the failed operation, rather than just showing a passive text message.
