...
**Learning:** Transient network failures (like failing to create a user profile or start a chat) displayed as passive `SnackBar`s force the user to manually retry the action, which requires navigating back to the form or button and tapping it again.
**Action:** Always provide an actionable recovery path (like a 'RETRY' button) directly within the `SnackBar` when a network or transient error occurs, so users can recover with a single tap. Note that adding an `onPressed` callback requires removing the `const` modifier from the `SnackBar` instantiation.
