...
## 2024-04-17 - Add Actionable Recoveries to Error Snackbars
**Learning:** Transient network failures shown via simple `SnackBar`s force users to manually re-initiate complex or state-heavy operations (like submitting forms or restarting a search/chat process), leading to high interaction friction.
**Action:** When displaying `SnackBar`s for network or transient errors, always remove the `const` modifier and include a `SnackBarAction` (e.g., 'RETRY') that directly re-invokes the failed operation without requiring the user to navigate away or re-enter data.
