...
## 2024-04-18 - Actionable Error SnackBars
**Learning:** Users experience friction when transient errors (like network failures during profile creation or starting a chat) require manual re-navigation or re-submitting forms.
**Action:** Always include a 'RETRY' `SnackBarAction` on error `SnackBar`s for async operations to provide an immediate, contextual recovery path.
