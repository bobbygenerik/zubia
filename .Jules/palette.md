...
**Learning:** When transient network failures happen (like failing to create a profile or start a chat), static error messages leave users stranded. They must manually re-navigate the UI to try the action again. Adding an actionable `RETRY` button to the error `SnackBar` completely transforms the error state into a seamless recovery flow, keeping users in context.
**Action:** Always provide an actionable `SnackBarAction` (like "RETRY") for errors that are transient and can be immediately re-attempted.
