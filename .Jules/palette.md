...
## 2024-05-29 - Actionable Recovery for Transient Failures
**Learning:** SnackBars that only display static error text for transient network failures (like "Could not create profile") force the user to manually re-initiate the action (e.g., tapping "Continue" again), increasing friction and cognitive load. Providing an actionable "RETRY" button directly in the SnackBar allows users to quickly recover without leaving the error context.
**Action:** Always add an actionable `SnackBarAction` (like 'RETRY') to `SnackBar`s for transient failures, and ensure the `SnackBar` instantiation is not `const` to allow for the `onPressed` callback.
