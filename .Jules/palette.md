...
## 2024-05-29 - Error Recovery with Actionable SnackBars
**Learning:** Transient failures (like network errors during profile or chat creation) are frustrating because they force users to manually re-navigate or re-enter data to try again. When a `SnackBar` simply says "Failed", the user is left in a dead end. By adding a `SnackBarAction` (like "RETRY") that re-triggers the failed operation, the error becomes a momentary speedbump rather than a roadblock.
**Action:** Always provide an actionable recovery path (like a `SnackBarAction` with "RETRY") for transient network or operational failures, ensuring the user can immediately try again without losing context or effort.
