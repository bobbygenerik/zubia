...
## 2024-05-29 - Actionable Error States
**Learning:** Transient failures (like network issues when creating a profile or starting a chat) presented as simple read-only `SnackBar`s force the user to re-navigate the UI or re-submit a form manually to try again. This causes friction. Providing an actionable `SnackBarAction` (e.g., "RETRY") directly on the error message allows immediate recovery and significantly reduces frustration.
**Action:** When displaying a `SnackBar` for a transient or retryable failure, always include a `SnackBarAction` that attempts the failed operation again.
