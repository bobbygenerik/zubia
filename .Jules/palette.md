...
**Learning:** Transient error messages (like network failures) shown via `SnackBar` are frustrating if they only inform the user of failure without offering a way to recover.
**Action:** Always include a `SnackBarAction` (like "RETRY") for transient operations to allow quick recovery without forcing the user to re-navigate the UI.
