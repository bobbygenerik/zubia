...
## 2024-05-29 - Add SnackBarAction for Transient Failures
**Learning:** Transient failures (like network issues during profile creation) create significant friction if the user has to re-initiate the action by finding the original button or re-navigating.
**Action:** Always add an actionable `SnackBarAction` (like 'RETRY') to error `SnackBar`s for transient failures so users can quickly recover in place.
