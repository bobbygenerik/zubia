...
## 2024-05-18 - Add retry actions to error notifications
**Learning:** Users often encounter transient network failures when creating a profile or starting a chat. Presenting a static error message requires them to dismiss the error and re-initiate the action manually, which increases friction.
**Action:** Add actionable `SnackBarAction` (e.g., 'RETRY') to error `SnackBar`s for transient failures, allowing users to quickly recover without re-navigating or re-entering data.
