...
## 2024-04-04 - Actionable SnackBars for Error Recovery
**Learning:** Adding an actionable 'RETRY' button to transient error SnackBars significantly improves micro-UX by allowing users to quickly recover without re-navigating or re-entering data. When implementing this in Flutter, remember to remove the `const` modifier from the `SnackBar` since the `onPressed` callback makes it a non-constant expression.
**Action:** Always include actionable SnackBarActions for error states where the user can meaningfully retry the failed operation.
