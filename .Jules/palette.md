...
## 2026-04-15 - Actionable Error Feedback
**Learning:** When transient network operations fail (like creating a profile or starting a chat), passive SnackBar error messages leave users stranded and require them to re-initiate complex flows from scratch. Adding an explicit 'RETRY' action significantly improves recovery UX by allowing immediate re-execution of the failed operation directly from the notification context.
**Action:** When implementing error notifications for async operations in Flutter, always provide a `SnackBarAction` (e.g., 'RETRY') paired with the original operation's callback function instead of just a text message, unless the error is permanently fatal.
