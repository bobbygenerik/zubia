...
## 2024-05-29 - Retry Actions in Transient Error Snackbars
**Learning:** When network requests or async operations fail (like creating a chat or updating a profile), standard error `SnackBar`s leave the user stuck. They must wait for the message to disappear, navigate back, or re-tap the originating button. Adding a simple `SnackBarAction` with "RETRY" significantly reduces friction and provides an immediate recovery path for transient failures.
**Action:** Always include a `SnackBarAction` (e.g., 'RETRY' or 'UNDO') on error notifications for actions that the user can immediately re-attempt without changing context. Ensure the `const` keyword is removed from the `SnackBar` instantiation so the `onPressed` callback functions correctly.
